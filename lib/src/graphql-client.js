...
export class GraphQLClient {
   ...
   *
   * @param {object} params
   * @param {string} params.url The GraphQL API url
   * @param {object} [params.headers] Global headers to add to every request.
   */
  constructor({ url, headers: globalHeaders }) {
    this.url = url;
  ...
  async query({ query, variables, headers }) {
    ...
    if (!res.ok || (json && json.errors)) {
      ...
      err.statusCode = extensions.statusCode;
      throw err;
    }
    return json;
  }

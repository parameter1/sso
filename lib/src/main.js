...
export default class SSO {
  constructor({
    ...
    if (!appId) throw new Error('No SSO application ID was provided.');
    this.appId = appId;
    this.origin = origin;
  ...
  getLoginPageUrl({ next = window.location.href } = {}) {
    return `${this.origin}/app/login?next=${encodeURIComponent(next)}&appId=${this.appId}`;
  }

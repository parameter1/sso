...
export default class SSO {
  ...
  /**
   * Retrieves the workspace for the provided app namespace.
   *
   * @param {object} params
   * @param {string} params.appNamespace
   * @param {boolean} [params.strict=true]
   * @returns {Promise<object>}
   */
  async getWorkspaceForApp({ appNamespace, strict = true }) {
    const { data } = await this.graphql.query({
      query: `
        query GetWorkspaceForApp {
          result: workspaceByNamespace(input: {
            namespace: "${this.appKey}/${appNamespace}"
            strict: ${strict}
          }) {
            _id name key namespace { application }
  ...
  async loadIframe() {
    if (!this.iframePromise) {
      this.iframePromise = loadIframe({ origin: this.origin });
    }
  ...
  async init() {
    if (!this.initPromise) {
      this.initPromise = (async () => {
        this.iframe = await this.loadIframe();

...
export class SSO {
  ...
  logout({ next } = {}) {
    window.location.href = this.getAppUrl({ path: '/logout', next });
  }
  ...
  getAppUrl({ path = '/', next } = {}) {
    ...
    return `${this.origin}${path}?next=${encodeURIComponent(`${url}`)}&appKey=${this.appKey}&_=${(new Date()).valueOf()}`;
  }

  redirectToApp({ next } = {}) {
    window.location.href = this.getAppUrl({ path: '/check-auth', next });
  }
